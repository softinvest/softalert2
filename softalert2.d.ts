declare module '@appscenter/softalert2' {
  /**
  ...
  namespace Swal {
    /**
     * Function to display a SoftAlert2 popup, with an object of options, all being optional.
     * See the `SoftAlertOptions` interface for the list of accepted fields and values.
     *
     * Example:
     * ```
     * Swal.fire({
     ...
     * ```
     */
    function fire<T = any>(options: SoftAlertOptions<T>): Promise<SoftAlertResult<Awaited<T>>>

    /**
     * Function to display a simple SoftAlert2 popup.
     *
     * Example:
     * ```
     ...
     * ```
     */
    function fire<T = any>(title?: string, html?: string, icon?: SoftAlertIcon): Promise<SoftAlertResult<Awaited<T>>>
     ...
     */
    function mixin(options: SoftAlertOptions): typeof Swal
     ...
     * ```
     */
    function update(options: Pick<SoftAlertOptions, SoftAlertUpdatableParameters>): void

    /**
     * Closes the currently open SoftAlert2 popup programmatically.
     *
     * @param result The promise originally returned by `Swal.fire()` will be resolved with this value.
     * If no object is given, the promise is resolved with an empty `SoftAlertResult` object.
     */
    function close(result?: Partial<SoftAlertResult>): void
     ...
     */
    function isValidParameter(paramName: string): paramName is keyof SoftAlertOptions
     ...
     * @param paramName The parameter to check
     */
    function isUpdatableParameter(paramName: string): paramName is SoftAlertUpdatableParameters

    /**
     * Normalizes the arguments you can give to Swal.fire() in an object of type SoftAlertOptions.
     *
     * Example:
     * ```
     ...
     */
    function argsToParams<T>(params: SoftAlertArrayOptions | readonly [SoftAlertOptions<T>]): SoftAlertOptions<T>
    ...
    /**
     * SoftAlert2's version
     */
    const version: string
  }

  interface SoftAlertHideShowClass {
  ...
  export type SoftAlertArrayOptions = readonly [string?, string?, SoftAlertIcon?]

  export type SoftAlertGrow = 'row' | 'column' | 'fullscreen' | false

  export type SoftAlertHideClass = SoftAlertHideShowClass

  export type SoftAlertShowClass = Readonly<SoftAlertHideShowClass>

  export type SoftAlertIcon = 'success' | 'error' | 'warning' | 'info' | 'question'

  export type SoftAlertInput =
    | 'text'
    | 'email'
    | 'password'
    | 'number'
  ...
  export type SoftAlertUpdatableParameters =
    | 'allowEscapeKey'
    | 'allowOutsideClick'
    | 'background'
    | 'buttonsStyling'
  ...
  export interface SoftAlertCustomClass {
  ...
  export interface SoftAlertResult<T = any> {
  ...
  export interface SoftAlertOptions<PreConfirmResult = any, PreConfirmCallbackValue = any> {
    /**
     ...
     * If `text` and `html` parameters are provided in the same time, `html` will be used.
     *
     * [Security] SoftAlert2 does NOT sanitize this parameter. It is the developer's responsibility
     ...
     * SoftAlert2 comes with 5 built-in icons which will show a corresponding icon animation:
    ...
    /**
     * Whether or not SoftAlert2 should show a full screen click-to-dismiss backdrop.
     ...
     * @default {}
     */
    customClass?: SoftAlertCustomClass | string
    ...
    /**
     * By default, SoftAlert2 sets html's and body's CSS `height` to `auto !important`.
    ...
    /**
     * If set to `false`, SoftAlert2 will allow keydown events propagation to the document.
     *
     * @default true
     */
    ...
    /**
     * Useful for those who are using SoftAlert2 along with Bootstrap modals.
     * By default keydownListenerCapture is `false` which means when a user hits `Esc`,
     * both SoftAlert2 and Bootstrap modals will be closed.
    ...
    /**
     * Whether to apply the default SoftAlert2 styling to buttons.
...
declare module '@appscenter/softalert2/*/softalert2.js' {
  export * from 'softalert2'
  // "export *" does not matches the default export, so do it explicitly.
  export { default } from 'softalert2' // eslint-disable-line
}

declare module '@appscenter/softalert2/*/softalert2.all.js' {
  export * from 'softalert2'
  // "export *" does not matches the default export, so do it explicitly.
  export { default } from 'softalert2' // eslint-disable-line
}
...
/**
 * These interfaces aren't provided by SoftAlert2, but its definitions use them.
