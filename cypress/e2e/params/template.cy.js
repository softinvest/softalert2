...
describe('template', () => {
  it('template as HTMLTemplateElement', () => {
    ...
    template.innerHTML = `
      ...
      <swal-image src="https://github.com/softinvest/softalert2/images/SoftAlert2.png" width="300" height="60" alt="safdsafd"></swal-image>
    ...
    expect(Swal.getImage().src).to.equal('https://github.com/softinvest/softalert2/images/SoftAlert2.png')
  ...
  it('should throw a warning when attempting to use unrecognized elements and attributes', () => {
    ...
    template.innerHTML = `
      ...
      <swal-image src="https://github.com/softinvest/softalert2/images/SoftAlert2.png" width="100" height="100" alt="" foo="1">Are you sure?</swal-image>
    ...
    expect(spy.getCall(0).calledWith(`SoftAlert2: Unrecognized element <swal-foo>`)).to.be.true
    expect(
      spy
        .getCall(1)
        .calledWith(
          `SoftAlert2: Unrecognized attribute "foo" on <swal-image>. Allowed attributes are: src, width, height, alt`
        )
    ).to.be.true
    expect(
      spy
        .getCall(2)
        .calledWith(
          `SoftAlert2: Unrecognized attribute "bar" on <swal-input>. Allowed attributes are: type, label, placeholder, value`
        )
    ).to.be.true
    expect(
      spy
        .getCall(3)
        .calledWith(
          `SoftAlert2: Unrecognized attribute "value" on <swal-title>. To set the value, use HTML within the element.`
        )
    ).to.be.true
  })
})
